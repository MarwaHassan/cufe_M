...
class FileManager(object):
    ...
    def __init__(
        ...
        print("I am this, basepath=%s" % basepath)
        if basepath[-1] == '/':
            basepath = basepath[:-1]
    ...
    def media(self, path):
        ...
        try:
            ...
            if 'filemanager' in path:
                print("FETCHING " + settings.FILEMANAGER_STATIC_ROOT + path)
            ...
            if not os.path.exists(imagepath):
                imagepath = (
                    settings.FILEMANAGER_STATIC_ROOT
                    + 'filemanager/images/icons/default.png'
                )
            print('Exception: %s' % imagepath)
    ...
    def render(self, request, path, new_path=''):
        ...
        if settings.FILEMANAGER_CHECK_SPACE:
                ...
                space_consumed = 0
        print("PATH: %s %s" % (new_path,new_path.split('/')[-1]))
        return render(
            request,
            'filemanager/index.html',
            {
                'root_dir': new_path.split('/')[-1],
